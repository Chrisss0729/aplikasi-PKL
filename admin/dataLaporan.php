                    ...
                    <div class="card-body bs-bg-opacity-0 p-0">
                        ...
                        <table class="table">
                            ...
                            <tbody class="table-group-divider">
                                ...
                                if (isset($_GET['cari'])) {
                                    $cari = $_GET['cari'];
                                    $data = mysqli_query($db, "SELECT * FROM tb_laporan INNER JOIN tb_siswa ON tb_laporan.nis = tb_siswa.NIS WHERE tb_siswa.nama LIKE '%$cari%'");
                                    $jumlah_data = mysqli_num_rows($data);
                                    $total_halaman = ceil($jumlah_data / $batas);
                                    $data_pegawai = mysqli_query($db, "SELECT * FROM tb_laporan INNER JOIN tb_siswa ON tb_laporan.nis = tb_siswa.NIS WHERE tb_siswa.nama LIKE '%$cari%' ORDER BY tb_laporan.id DESC limit $halaman_awal, $batas ");
                                    $nomor = $halaman_awal + 1;
                                } else {
                                    $data = mysqli_query($db, "select * from tb_laporan");
                                    $jumlah_data = mysqli_num_rows($data);
                                    $total_halaman = ceil($jumlah_data / $batas);
                                    $data_pegawai = mysqli_query($db, "SELECT * FROM tb_laporan INNER JOIN tb_siswa ON tb_laporan.nis = tb_siswa.NIS ORDER BY tb_laporan.id DESC limit $halaman_awal, $batas ");
                                    $nomor = $halaman_awal + 1;
                                }
